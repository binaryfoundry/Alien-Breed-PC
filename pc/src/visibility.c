...
/* -----------------------------------------------------------------------
 * order_zones - Build front-to-back zone draw order
 *
 ...
 * 3. For each marked room, calculate distance from viewer using
 *    dot product against floor line normals
 * 4. Insert into sorted linked list (front-to-back)
 * 5. Build FinalOrder output array
 * ----------------------------------------------------------------------- */
                 ...
                 const uint8_t *list_of_graph_rooms)
{
    ...
    /* Step 2: For each marked zone, calculate distance from viewer */
    for (int z = 0; z < 256; z++) {
        ...
        /* Get exit list: ToExitList is a 16-bit RELATIVE offset from zone_data
         * (ASM: adda.w ToExitList(a0),a0) */
        int16_t exit_rel = read_be16(zone_data + ZONE_EXIT_LIST);
        if (exit_rel == 0) continue;
        const uint8_t *exit_list = zone_data + exit_rel;
        int16_t first_line = read_be16(exit_list);
        if (first_line < 0) continue;

        if (!level->floor_lines) continue;
        const uint8_t *fline = level->floor_lines + first_line * FLINE_SIZE;
        ...
        /* Distance = dot product of (viewer - line_origin) with line normal
         * Normal is perpendicular to line direction: (zlen, -xlen)
         * dist = (viewer_x - lx) * zlen - (viewer_z - lz) * xlen
         */
        int32_t dx = (int32_t)(viewer_x - lx);
        int32_t dz = (int32_t)(viewer_z - lz);
        int32_t dist = dx * lzlen - dz * lxlen;

        zone_ids[num_zones] = (int16_t)z;
        distances[num_zones] = dist;
        num_zones++;

        if (num_zones >= MAX_ORDER_ENTRIES) break;
    }

    /* Step 3: Sort by distance (simple insertion sort - original uses linked list) */
