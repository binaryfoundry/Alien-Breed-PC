...
#define STEP_UP_NORMAL      (40 * 256)
#define STEP_UP_DUCKED      (10 * 256)
#define STEP_DOWN_DEFAULT   0x1000000
 ...
 * Friction helper
 *
 * The original applies friction differently per control mode:
 *   Mouse: asr.l #1 (divide by 2) with rounding fix
 *   Keyboard: asr.l #3 (divide by 8) with rounding fix
 *
 * The "rounding fix" is:
 *   neg.l d6; ble .nobug; asr #N,d6; add #1,d6; bra .done
 *   .nobug: asr #N,d6
 * This rounds toward zero for negative values.
 * ----------------------------------------------------------------------- */
                                     ...
                                     const KeyBindings *keys, int16_t temp_frames)
{
    ...
    /* ---- Velocity friction (divide by 8) ---- */
    int32_t xspd = apply_friction(plr->s_xspdval, 3);
    int32_t zspd = apply_friction(plr->s_zspdval, 3);
                                  ...
                                  const KeyBindings *keys, int16_t temp_frames)
{
    ...
    /* Velocity friction (divide by 2 for mouse mode) */
    int32_t xspd = apply_friction(plr->s_xspdval, 1);
    int32_t zspd = apply_friction(plr->s_zspdval, 1);
                                      ...
                                      const KeyBindings *keys, int16_t temp_frames)
{
    ...
    /* ---- Velocity friction (divide by 4 for mouse+kbd: snappier than
     *      pure keyboard /8, less slippery than pure mouse /2) ---- */
    int32_t xspd = apply_friction(plr->s_xspdval, 2);
    int32_t zspd = apply_friction(plr->s_zspdval, 2);
...
static void player_full_control(PlayerState *plr, GameState *state, int plr_num)
{
     ...
     *     objroom points to the zone data for the player's current room.
     *     Translated from AB3DI.s: PLR1_Roompt -> ctx.objroom */
    ctx.objroom = NULL;
